...
export default function YeniMusteriGorusuPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Müşteri görüşü eklenirken bir hata oluştu');
      }
      ...
      if (err instanceof Error) {
        ...
        setError('Müşteri görüşü eklenirken bir hata oluştu');
      }
      console.error('Müşteri görüşü eklenirken hata:', err);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold">Yeni Müşteri Görüşü Ekle</h1>
        <p className="text-gray-600">Müşteri görüşlerini buradan ekleyebilirsiniz.</p>
      </div>
