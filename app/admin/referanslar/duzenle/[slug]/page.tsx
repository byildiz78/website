...
export default function DuzenleReferansPage({ params }: { params: { slug: string } }) {
  ...
  const [originalData, setOriginalData] = useState<ReferansFormData | null>(null);
  ...
  useEffect(() => {
    async function fetchReferans() {
      try {
        ...
        // API'den belirli bir referansı getir
        const response = await fetch(`/api/admin/referanslar/${encodeURIComponent(decodedSlug)}`, {
          headers: {
        ...
        if (!response.ok) {
          throw new Error('Referans verileri yüklenirken bir hata oluştu');
        }
        ...
        setFormData(data.referans);
        setOriginalData(data.referans);
        
        // Logo önizlemesini ayarla
        if (data.referans.logo_yolu) {
          setPreviewImage(`/${data.referans.logo_yolu}`);
        }
      } catch (error) {
        console.error('Referans yüklenirken hata oluştu:', error);
        if (error instanceof Error) {
          setError(error.message);
        } else {
          setError('Referans yüklenirken bir hata oluştu. Lütfen daha sonra tekrar deneyin.');
        }
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!originalData) {
        throw new Error('Orijinal referans verileri bulunamadı');
      }
      ...
      const response = await fetch('/api/admin/referanslar', {
        ...
        body: JSON.stringify({
          originalName: originalData.adi,
          updatedReferans: formData
        })
      });
      
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Referans güncellenirken bir hata oluştu');
      }
  ...
  if (loading) {
    return (
      <div className="p-8 flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p className="mt-4 text-gray-600">Referans bilgileri yükleniyor...</p>
      </div>
    );
  }

  if (error && !formData.adi) {
    return (
      <div className="p-8">
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Referans Düzenle: {formData.adi}</h1>
        <button
          onClick={() => router.push('/admin/referanslar')}
