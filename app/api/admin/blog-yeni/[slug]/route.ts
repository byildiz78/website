...
import { getNewBlogPostBySlug, updateNewBlogPost, deleteNewBlogPost } from '@/lib/db-new';
...
export async function GET(
  ...
  try {
    ...
    const { slug } = params;
    const post = await getNewBlogPostBySlug(slug);
...
export async function PUT(
  ...
  try {
    ...
    const updatedPost = await updateNewBlogPost(slug, data);
...
export async function DELETE(
  ...
  try {
    ...
    const { slug } = params;
    const success = await deleteNewBlogPost(slug);
